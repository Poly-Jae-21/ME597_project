...
class Microgrid:
	def __init__(self, name, load_parameters, battery_parameters):
		...
		self.houses, self.schools, self.church, self.health_centers, self.water_pumps= self._create_loads(load_parameters)
...
#creates the loads of the MG, using the number of units for each load and its name for data reasons
	def _create_loads(self, load_parameters):
		num_houses, num_schools, num_churches, num_health_centers, num_water_pumps = load_parameters
		houses_load = Load("House", HOUSE_MAX_LOAD, num_houses)
		schools_load = Load("School", SCHOOL_MAX_LOAD, num_schools)
		churches_load = Load("Mosque", MOSQUE_MAX_LOAD, num_churches)
		...
		return houses_load, schools_load, churches_load, health_centers_load, water_pumps_load
...
#returns the total load by all MG load units
	def total_load(self, time):
		...
		churches_load = self.church.current_total_load(time)
		health_centers_load = self.health_centers.current_total_load(time)
		water_pumps_load = self.water_pumps.current_total_load(time)
		total_load = houses_load + schools_load + churches_load + health_centers_load + water_pumps_load
		return total_load
...
class MicrogridEnv (gym.Env):
	def __init__(self):
		...
		self.action_space = spaces.Box(low=np.array([0,0,0,self.main_mG.unit_price,0]), high=np.array([3, 2, self.main_mG.battery.max_capacity, NETWORK_PRICE,3]), dtype = np.float32)
